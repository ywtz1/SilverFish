namespace HREngine.Bots
{
    ...
    public class PenalityManager
    {
            ...
            if (target.own && !target.isHero)
            {
                ...
                if (DamageTargetSpecialDatabase.ContainsKey(name))
                {
                    ...
                    switch (name)
                    {
                        ...
                        if (m.Hp >= 2 &&m.own) return 5; 
                        if (m.Hp >= 3 &&m.own) return 2; 
                        break;
        ...
        public int discoverCardPriority(Playfield p ,List<Handmanager.Handcard> discoverCards)//发现卡牌的选择
        {
            ...
            {
                CardDB.Card c = discoverCards[i].card;
                if (c.cost<p.mana)
                {
                    if (this.cardvaluefornow(p,c)>cardvalue)
                    {
                        Choice = i;
                        cardvalue = this.cardvaluefornow(p,c);
                    }
                }
                else 
                {
                    if (this.cardvaluefornow(p,c)/2 > cardvalue)
                    {
                        Choice = i;
                        cardvalue = this.cardvaluefornow(p,c)/2;
                    }
        ...
        public int CardDrawsimDeck(Playfield p )//根据牌库进行计算抽牌价值对原算法进行修正
        {
            ...
            foreach (KeyValuePair<CardDB.cardIDEnum, int> e in deck)
            {
                ...
                int n =e.Value;
                if (c.cost<p.mana)cardvalue += cardvaluefornow(p,c)*n;

                //if(Probabilitymaker.Instance.ownCardsOut.ContainsKey(e.Key))n-= Probabilitymaker.Instance.ownCardsOut[e.Key];

            }
            return  (int)(cardvalue/30-2);
        }
///********************************************************************
        public double cardvaluefornow(Playfield p ,CardDB.Card c) 
        {
                double cardvalue=0;
                //if (c.cost<p.mana)
                {
                    ...
                    {
                        if(this.heroAttackBuffDatabase.ContainsKey(c.name)) cardvalue +=this.heroAttackBuffDatabase[c.name];
                        if(this.attackBuffDatabase.ContainsKey(c.name)) cardvalue +=this.attackBuffDatabase[c.name];
                        if(this.DamageAllDatabase.ContainsKey(c.name)) cardvalue +=this.DamageAllDatabase[c.name];
                        if(this.DamageAllEnemysDatabase.ContainsKey(c.name)) cardvalue +=this.DamageAllEnemysDatabase[c.name];
                    ...
                    if(this.CardNeedNow(p,2)) 
                    {
                        if(this.heroAttackBuffDatabase.ContainsKey(c.name)) cardvalue +=this.heroAttackBuffDatabase[c.name];
                        if(this.attackBuffDatabase.ContainsKey(c.name)) cardvalue +=this.attackBuffDatabase[c.name];
                        if(this.DamageHeroDatabase.ContainsKey(c.name)) cardvalue +=this.DamageHeroDatabase[c.name];
                        if(this.DamageRandomDatabase.ContainsKey(c.name)) cardvalue +=this.DamageRandomDatabase[c.name]/3;
                        if(this.DamageTargetDatabase.ContainsKey(c.name)) cardvalue +=this.DamageTargetDatabase[c.name];
                    ...
                    if(this.CardNeedNow(p,3))
                    {
                        ...
                        if(this.ownSummonFromDeathrattle.ContainsKey(c.name)) cardvalue +=this.ownSummonFromDeathrattle[c.name]/-10;
                        if(c.type==CardDB.cardtype.MOB)cardvalue += 2 + (c.Attack+ c.Health +c.cost)/2;
                        
                    }
                    if(this.CardNeedNow(p,4))
                    {
                        if(this.HealTargetDatabase.ContainsKey(c.name)) cardvalue +=this.HealTargetDatabase[c.name]/2;
                        if(this.HealAllDatabase.ContainsKey(c.name)) cardvalue +=this.HealAllDatabase[c.name]/2;
                    }
        ...
        public bool CardNeedNow(Playfield p,int n)//1、伤害牌 2、斩杀 3、召唤随从或随从牌  4、回血或嘲讽 5、AOE 
        {
            ...
            if(n == 2 && p.enemyHero.Hp < 10) return true;
            ...
            if(n == 4 ) 
            {
                ...
                if(p.ownHero.Hp < 10)return true;
            }
        ...
        private int getDestroyOwnPenality(CardDB.cardName name, Minion target, Playfield p)//消灭我方随从
        {
            ...
            switch (name)
            {
                case CardDB.cardName.evilgenius: //怪盗天才
                    ...
                    if(target.Hp <= 2)nnn-=5-target.Hp;
            ...
            if (card.Secret)
            {
                ...
                switch (card.name)
                {
                    ...
                    case CardDB.cardName.iceblock: 
                        if(p.ownHero.Hp <15)pen-=8;break; 
        ...
        private int getAttackSecretPenality(Minion m, Playfield p, Minion target)
        {
            ...
            if (p.enemyHeroStartClass == TAG_CLASS.MAGE)
            {
                if (target.isHero)
                {
                    ...
                    else if(canBe_flameward)
                    {
                        ...
                        foreach (Minion mnn in p.ownMinions)
                        {
                            ...
                            if(mnn.Hp<4&&att1<mnn.Attack){att1=mnn.Attack;maxatt4hp=mnn;}
                        }
                ...
                else
                {
                    ...
                    {
                        pen = 1;
                        if (target.Hp > m.Attack || target.divineshild) return 0;
                        else
                        {
