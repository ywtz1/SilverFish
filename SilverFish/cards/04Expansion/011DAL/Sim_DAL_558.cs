    ...
    class Sim_DAL_558 : SimTemplate //* 大法师瓦格斯
	{
        ...
        public override void onTurnEndsTrigger(Playfield p, Minion triggerEffectMinion, bool turnEndOfOwner)
        {
            ...
            if (triggerEffectMinion.own == turnEndOfOwner&&turnEndOfOwner && n>0 && spell !=null&&spell.name!=CardDB.cardName.unknown)
            {
                ...
                spell.CardSimulation.onCardPlay(p,turnEndOfOwner,(p.ownMinions.Count!=0||p.ownMinions.Count!=0)?(p.ownMinions.Count>=p.ownMinions.Count? p.searchRandomMinion(p.ownMinions, searchmode.searchHighestAttack):p.searchRandomMinion(p.enemyMinions, searchmode.searchHighestAttack)):p.enemyHero,0);
            }
        }
